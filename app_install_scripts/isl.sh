...
SRC_URL="http://isl.gforge.inria.fr/isl-0.16.1.tar.gz"
PKG_URL=NULL
DISTRIBUTION=NULL
rst=0

## Selfdef Varis
MY_SRC_DIR="isl-0.16.1"
MY_SRC_TAR="isl-0.16.1.tar.gz"
...
function install_depend()
{
	if [ "${DISTRIBUTION}"x == "Debian"x ] ; then
		...
		ass_rst $? 0 "apt-get liggmp-dev failed!"
	elif [ "${DISTRIBUTION}"x == "CentOS"x ] ; then
		yum install -y freetype-devel.aarch64 gmp-devel --setopt=skip_missing_names_on_install=False
		ass_rst $? 0 "yum freetype-devel.aarch64 failed!"
	fi
	pr_ok "[depend] OK"
	return 0
}
...
function selftest()
{
	...
	if [ ! -f "/usr/local/lib/libisl.so" ] ; then
		ass_rst 1 0 "isl install failed!"
	fi
