...
## name:	busybox
## desc:	busybox  source code compile and install or busybox  package install
...
function install_depend()
{
	case $DISTRIBUTION in
		"CentOS")
			DEPENDENCE="wget tar bzip2 automake autoconf libtool make gcc gcc-c++"
...
function download_src()
{
	case $DISTRIBUTION in
		"CentOS")
			...
			wget https://busybox.net/downloads/busybox-1.22.0.tar.bz2 -O $filename.tar.bz2
			ass_rst $? 0 "download failed"
			tar -xf $filename.tar.bz2
			ass_rst $? 0 "unpack source file failed"
			;;	
		"Debian")
