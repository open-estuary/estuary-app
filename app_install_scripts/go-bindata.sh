...
## name:	go-bindata
## desc:	go-bindata package install
...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		apt-get install -y go-bindata
		ass_rst $? 0 "apt-get install go-bindata failed!"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		rm -rf go-bindata-3.0.7-14.gita0ff256.fc29.aarch64.rpm
		wget http://www.rpmfind.net/linux/fedora/linux/releases/29/Everything/aarch64/os/Packages/g/go-bindata-3.0.7-14.gita0ff256.fc29.aarch64.rpm
		rpm -i go-bindata-3.0.7-14.gita0ff256.fc29.aarch64.rpm
		ass_rst $? 0 "rpm install go-bindata-3.0.7-14.gita0ff256.fc29.aarch64.rpm failed!"
		rm -rf go-bindata-3.0.7-14.gita0ff256.fc29.aarch64.rpm
	fi
...
{
	go-bindata --version
	ass_rst $? 0 "test go-bindata failed!"
	pr_ok "[selftest] OK"
	return 0
}
