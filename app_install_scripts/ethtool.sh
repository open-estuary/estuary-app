...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		...
		ass_rst $? 0 "apt-get ethtool failed!"
		pr_ok "[compile]<install> ok"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		yum install -y ethtool --setopt=skip_missing_names_on_install=False
		ass_rst $? 0 "yum install ethtool failed!"
		pr_ok "[compile]<install> ok"
	fi
