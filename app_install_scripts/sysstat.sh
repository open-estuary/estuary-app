...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		...
		ass_rst $? 0 "sysstat install failed"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		yum install -y sysstat --setopt=skip_missing_names_on_install=False
...
{
	sar -V
	ass_rst $? 0 "selftest sysstat failed!"
