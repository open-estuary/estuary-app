...
## desc:	mysql-community package install
...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		...
		ass_rst $? 0 "download mysql-community source rpm failed"
		yum install ./mysql80-community-release-el7-1.noarch.rpm
		ass_rst $? 0 "install mysql-community rpm failed"
		pr_info "install using yum"
		yum --setopt=skip_missing_names_on_install=False install -y mysql-community-server
		pr_ok "[compile]<install> ok"
	fi
	ass_rst $? 0 "install mysql-community failed"
