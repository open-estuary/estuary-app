...
## desc:	install python-cssselect package
...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		apt-get install -y python-cssselect
		ass_rst $? 0 "apt-get failed!"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		rm -rf python-cssselect-0.9.1-5.el7.noarch.rpm

		wget http://www.rpmfind.net/linux/epel/7/aarch64/Packages/p/python-cssselect-0.9.1-5.el7.noarch.rpm
		ass_rst $? 0 "wget failed!"

		rpm -i python-cssselect-0.9.1-5.el7.noarch.rpm
		ass_rst $? 0 "install python-cssselect-0.9.1-5.el7.noarch.rpm failed!"
