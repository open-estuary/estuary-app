...
function install_depend()
{
	...
	if [ "$DISTRIBUTION"x == "CentOS"x ]
	then
		yum --setopt=skip_missing_names_on_install=False install epel-release -y
		yum --setopt=skip_missing_names_on_install=False install -y --enablerepo=extras centos-release-ceph

		#repo verify
		sed -i '9,10d' /etc/yum.repos.d/CentOS-Ceph-Luminous.repo
		sed -i '8 agpgcheck=0\nenabled=0' /etc/yum.repos.d/CentOS-Ceph-Luminous.repo
		sed '12 a[ceph]\nbaseurl=http://download.ceph.com/rpm-mimic/el7/aarch64/\nenabled=1\ngpgcheck=0\n' -i /etc/yum.repos.d/CentOS-Ceph-Luminous.repo

		mkdir /usr/lib/python2.7/site-packages/urllib3/packages/ssl_match_hostname_bk > /usr/lib/python2.7/site-packages/urllib3/packages/mkdir.log
		mv /usr/lib/python2.7/site-packages/urllib3/packages/ssl_match_hostname /usr/lib/python2.7/site-packages/urllib3/packages/ssl_match_hostname_bk

		yum --setopt=skip_missing_names_on_install=False install -y ceph-13.2.2
		ass_rst $? 0 "install using yum failed"
		pr_ok "[depend] ok"
	elif [ "$DISTRIBUTION"x == "Debian"x ]
	then
