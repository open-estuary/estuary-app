...
## name:	snappy
## desc:	snappy source code build and install
...
SRC_URL="https://github.com/google/snappy/archive/1.1.4.tar.gz"
PKG_URL=NULL
DISTRIBUTION=NULL
rst=0

## Selfdef Varis
MY_SRC_DIR="snappy-1.1.4"
MY_SRC_TAR="1.1.4.tar.gz"
...
function install_depend()
{
	if [ "${DISTRIBUTION}"x == "Debian"x ] ; then
		apt-get install -y libtool maven libsysfs-dev
		pr_ok "[depend] skiped"
...
function compile_and_install()
{
	...
	#aclocal -I m4
	./autogen.sh
	ass_rst $? 0 "autogen.sh exe failed"
...
function selftest()
{
	if [ -f "/usr/local/lib/libsnappy.a" ] ; then
		...
		ass_rst 1 0 "check libsnappy.a failed!"
	fi
