...
## desc:	source-highlight package install
...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		...
		ass_rst $? 0 "source-highlight install failed"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		yum install -y source-highlight --setopt=skip_missing_names_on_install=False
