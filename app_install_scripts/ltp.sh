...
SRC_URL="https://github.com/linux-test-project/ltp/releases/download/20180926/ltp-full-20180926.tar.bz2"
PKG_URL=NULL
DISTRIBUTION=NULL
rst=0

## Selfdef Varis
MY_SRC_DIR="ltp-full-20180926"
MY_SRC_TAR="ltp-full-20180926.tar.bz2"
...
{
	ltpServer
	ass_rst $? 0 "ltp install test failed!"	
...
function compile_and_install()
{
	...
	make autotools
	ass_rst $? 0 "make_autotools failed"
	...
	make all
	ass_rst $? 0 "make all failed"

	make "DESTDIR=/" SKIP_IDCHECK=1 install
	ass_rst $? 0 "make install failed"
	
	echo $PATH | grep "/opt/ltp"
		if [ $? != 0 ] ; then
		echo 'export PATH=$PATH:/opt/ltp'>> /etc/profile
		source /etc/profile
		pr_info "path create success!"
	else
		pr_info "path already exist!"
	fi
