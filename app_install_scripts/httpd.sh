...
## name:	httpd
## desc:	httpd/apache package install
...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		...
		ass_rst $? 0 "apt-get install apache2 failed!"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		yum install -y httpd --setopt=skip_missing_names_on_install=False
		ass_rst $? 0 "yum install httpd failed!"
	fi
