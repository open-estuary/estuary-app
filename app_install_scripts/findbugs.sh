...
function install_depend()
{
	...
	if [ "${DISTRIBUTION}"x == "CentOS"x ] ; then
		yum install -y dom4j findbugs-bcel jFormatString jaxen jcip-annotations jsr-305 --setopt=skip_missing_names_on_install=False
		ass_rst $? 0 "yum install depend failed!"
		pr_ok "[depend] OK"
	else
		pr_tip "[denpend] skiped"
	fi
...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		apt-get install -y findbugs
		ass_rst $? 0 "apt-get findbugs failed!"
		pr_ok "[compile]<install> ok"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		rm -rf findbugs-3.0.1-15.fc29.noarch.rpm
		wget http://www.rpmfind.net/linux/fedora/linux/releases/29/Everything/aarch64/os/Packages/f/findbugs-3.0.1-15.fc29.noarch.rpm
		rpm -i findbugs-3.0.1-15.fc29.noarch.rpm
		ass_rst $? 0 "rpm install findbugs failed!"
		rm -rf findbugs-3.0.1-15.fc29.noarch.rpm
		pr_ok "[compile]<install> ok"
	fi
