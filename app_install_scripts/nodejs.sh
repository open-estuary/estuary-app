...
function compile_and_install()
{
	...
	if [ "$DISTRIBUTION"x == "Debian"x ] ; then
		...
		ass_rst $? 0 "install deb source failed"
		pr_info "install using apt-get"
		apt-get install -y nodejs
		ass_rst $? 0 "install failed"
		rm /etc/apt/sources.list.d/nodesource.list
		pr_error "delete nodesource.list failed"
	elif [ "$DISTRIBUTION"x == "CentOS"x ] ; then
		yum --setopt=skip_missing_names_on_install=False install -y nodejs
		ass_rst $? 0 "install failed"
	fi
